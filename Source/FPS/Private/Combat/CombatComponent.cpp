...
float UCombatComponent::CalculateDamageForHit(bool bHeadShot) const
{
	if (!IsValid(CurrentWeapon)) return 0.f;
	
	const float BaseDamage = CurrentWeapon->Damage;
	if (!bHeadShot || !IsValid(WeaponData))
	{
		return BaseDamage;
	}
	return BaseDamage * WeaponData->GetHeadShotDamageMultiplier(CurrentWeapon->WeaponType);
}
